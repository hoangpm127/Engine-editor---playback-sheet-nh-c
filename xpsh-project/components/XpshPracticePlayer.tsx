/**
 * XPSH Practice Player Component
 * Piano playback với A-B loop và seek to measure
 */
...
import { LoopTimeline, LoopRange } from '@/components/LoopTimeline';
...
export interface XpshPracticePlayerProps {
  score: XPSHScore;
  autoPlay?: boolean;
}

export function XpshPracticePlayer({ score, autoPlay = false }: XpshPracticePlayerProps) {
  // ========================================================================
  // State
  // ========================================================================
  ...
  useEffect(() => {
    ...
    if (loopRange.measureA >= loopRange.measureB) return; // Invalid range

    // Calculate loop points in ms
    const loopStartMs = measureToMs(loopRange.measureA, tempo);
    const loopEndMs = measureToMs(loopRange.measureB + 1, tempo); // +1 because B is inclusive

    // Check if we've passed the loop end point
    if (state.currentMs >= loopEndMs) {
      // Seek back to loop start
      controls.seek(loopStartMs);
    }
  }, [state.currentMs, state.isPlaying, loopEnabled, loopRange, tempo, controls]);

  // ========================================================================
  // Loop Timeline Handlers
  // ========================================================================

  const handleSetA = useCallback((measure: number) => {
    setLoopRange(prev => ({ ...prev, measureA: measure }));
  }, []);

  const handleSetB = useCallback((measure: number) => {
    setLoopRange(prev => ({ ...prev, measureB: measure }));
  }, []);
  ...
  const handleSeekToMeasure = useCallback((measure: number) => {
  ...
  return (
    <div className="xpsh-practice-player" style={styles.container}>
      ...
      {/* Loop Timeline */}
      <div style={styles.loopTimelineContainer}>
        <LoopTimeline
          ...
          onSeekToMeasure={handleSeekToMeasure}
      ...
      <div style={styles.controls}>
        <button
          ...
          style={{
            ...styles.button,
            ...(state.isPlaying ? styles.buttonDisabled : {})
          }}
        >
          ▶ Play
        </button>
      ...
      <div style={styles.info}>
        ...
        <div>Loop: {loopEnabled ? '🔁 ON' : '🔁 OFF'}</div>
